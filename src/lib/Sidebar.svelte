...
let slider: UI_Slider = { index:0, min: 0, max: 1, value: 0.5, step: 0.01 };

  $ParamDefsHost.map(({ paramId, min, max, defaultValue }, index) => {
    slider = { index, min, max, value: defaultValue, step: 0.01 };
  $UI_Controls.set(paramId, slider);
});

 // add extra params that are ViewState related only, not for controlling audio params the host
 $UI_Controls.set("smooth", { index: 5, min: 0, max: 1, value: 0.5, step: 0.01 });
</script>

<Controls controlStore={UI_Controls} />
<ParameterLock />
